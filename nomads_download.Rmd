...
file = list.files("./flowline-app/data/current_nc", full.names = F)

if(length(file) > 0){
  ttt = strsplit(list.files("./flowline-app/data/current_nc", full.names = F), "_")[[1]]
  date = ttt[1]
  time = ttt[2]
  forecast = ttt[3]
} else {
...
if(all(
  ...
  file.remove(list.files("./flowline-app/data/current_nc", full.names = T))
